...
combined_databike_2 <- mutate(combined_databike_2, ...1 = NULL)

combined_databike_2 <- combined_databike_2 %>% 
  select(-start_lat,-start_lng,-end_lat,-end_lng)
...
combined_databike_2 %>% 
  ...
  scale_y_continuous(labels = label_number(suffix = " M", scale = 1e-6)) +
  geom_text(stat="", aes(label=..count..), vjust=+2, color="white")
...
combined_databike_2 %>% 
  ...
  geom_col(position = "dodge") + labs(title= "Numbers of rides completed by month by member_casual",
                                        subtitle= "June 2020 to May 2021")  
...
ggplot(Table2.1,aes(x = end_station_name, y = number_of_rides, fill = member_casual)) +
  geom_bar(stat = "identity", width = 0.2) +
  coord_flip() +
  labs(title = "Top 10 end station by casual customer")
## top 10 start station by member customer
ggplot(Table2.2,aes(x = end_station_name, y = number_of_rides, fill = member_casual)) +
  geom_bar(stat = "identity", width = 0.2) +
  coord_flip() +
  labs(title = "Top 10 end station name by member customer")
